...
ggplot(summary_results, aes(x = feature_num, y = mean_rank, color = method, group = method)) +
  ...
  scale_y_continuous(breaks = 1:10, trans = "reverse", limits = c(10, 1)) +
...
```{r}
ggplot(importance_df_long, aes(x = Rank, y = reorder(Feature, Rank), color = Method)) +
  geom_point(size = 3) +
  geom_text(aes(label = Rank), hjust = -0.4, size = 3) +
  labs(
    title = "Comparison of Feature Importance Rankings (OOB Perm vs VIπᴸⱼ)",
    x = "Rank (lower = more important)",
    y = "Feature"
  ...
  scale_x_reverse(breaks = 1:max(importance_df_long$Rank)) +
